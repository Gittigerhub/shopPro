...
public class ItemsearchRepositoryImpl extends QuerydslRepositorySupport implements ItemsearchRepository {
    ...
    public Page<Item> getAdminItemPage(PageRequestDTO pageRequestDTO, Pageable pageable, String email) {
        // 만들 기능들 : 판매중 풀절 여부, 날짜에 따른 검색, 만든이 (중요), 아이템 이름
        ...
        // or은 계속해서 찾아내고 and는 하나만 찾아냄
        if (searchDateType == null || searchDateType.equals("all") || searchDateType.equals("")) {

        }else if (searchDateType.equals("1d")) {
            booleanBuilder.and(item.regTime.after(localDateTime.minusDays(1)));     // 하루 전
        }else if (searchDateType.equals("1w")) {
            booleanBuilder.and(item.regTime.after(localDateTime.minusWeeks(1)));    // 1주 전
        }else if (searchDateType.equals("1m")) {
            booleanBuilder.and(item.regTime.after(localDateTime.minusMonths(1)));   // 1달 전
        }else if (searchDateType.equals("6m")) {
            booleanBuilder.and(item.regTime.after(localDateTime.minusMonths(6)));   // 6달 전
        }
...
//                if(str.equals("t"))
                switch (str){
                    case "n" :              // 상품 이름
                        booleanBuilder.or(item.itemNm.contains(keyword));       // 키워드가 이름과 같은 것
                        break;
                    case "d" :              // 상품 상세설명
                        booleanBuilder.or(item.itemDetail.contains(keyword));   // 디테일이 키워드와 같은 것
                        break;
//                    case "c" :              // createBy
//                        booleanBuilder.or(item.createBy.contains(keyword));     // 키워드가 작성한것
//                        break;
